  ...
  const fetchRecipe = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.edamam.com/search?q=${query}&app_id9dbc2cc5=&app_key=4ec3d14e625f80b8db09ca5e14926c7b
   `
      );
    ...
    <>
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 place-items-center gap-4 p-4 bg-slate-200">
          {recipes.map((recipe) => (
            <div key={recipe.recipe.label} className="shadow-lg rounded-md p-2">
              <img
